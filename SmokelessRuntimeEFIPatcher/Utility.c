...
)
{
    ...
    for (UINTN Index = 0; Index < HandleBufferSize; Index++)
    {
        ...
        while (TRUE)
        {
            ...
            UINT32 AuthenticationStatus;
            String = NULL;
            FvInstance->ReadSection(FvInstance, &NameGuid, EFI_SECTION_USER_INTERFACE, 0, &String, &StringSize, &AuthenticationStatus);
...
)
{
    ...
    for (UINTN Index = 0; Index < BufferSize; Index++)
    {
        ...
        gBS->LoadImage(FALSE, gImageHandle, FilePath, (VOID *)NULL, 0, AppImageHandle);
        if (FilePath != NULL) {
            FreePool(FilePath);
        }
...
)
{
  if (isUnformattedAscii)
  {
    for (UINT64 i = 0; i < Size * 2; i++)
    {
      ...
      UnicodeSPrint(Log, GenericBufferSize, u"%c", Dump[i]);
      LogToFile(LogFile, Log);
    }
    UnicodeSPrint(Log, GenericBufferSize, u"%a", "\n\t");
    LogToFile(LogFile, Log);
...
)
{
  ...
  //If the very first argument is our terminator (NOP), the list is empty
  if (First == 0) {
    return FALSE;
  }
  ...
  //Loop until the sentinel value (0) is encountered
  while (TRUE) {
    CurrentValue = VA_ARG(Marker, UINTN);
    if (CurrentValue == 0) {
      break;
    }
