      ...
      *BaseOffset = CurrentOffset;
      ...
      if (CurrentOffset > 0) {
        *isOpSkipAllowed = TRUE;
        ...
        PrintConsoleSREP(HiiGetStringSREP(STRING_TOKEN(STR_PATCHED), NULL));
      ...
      {
        PrintConsoleSREP(HiiGetStringSREP(STRING_TOKEN(STR_NOT_PATCHED), NULL));
      ...
      if (SearchType == PATTERN && CurrentOffset)
      {
        ...
        for (UINTN i = 0; (i < ImageInfo->ImageSize - PatternLength) && PatchLimit; i++)
        {
          ...
          if (CResult)
          {
            ...
            PrintDump(PatternLength, (UINT8 *)ImageInfo->ImageBase + i);
      ...
      if (MatchesCount) {
        ...
        *isOpSkipAllowed = TRUE;
        ...
        for (UINTN i = 0; i < MatchesCount; i++) {
          ...
          PrintDump(PatternLength, (UINT8 *)ImageInfo->ImageBase + Captures[i]);
        }

        if (!IsZeroBuffer(HexPatch, PatchLength)) {
          *Status = EFI_SUCCESS;
          FreePool(ProcessedPatch);
          FreePool(HexPatch);
